...
/**
 * SYNTHETIC MOTOR INSURANCE POLICY — DEMO KNOWLEDGE BASE
 * Standard Comprehensive Motor Vehicle Policy (Tariff Endorsements & General Regulations)
 * 
 * Each clause strictly conforms to the demo specification:
 * - Clause ID
 * - Clause title
 * - Policy text (Rule)
...
export const MOTOR_POLICY_KNOWLEDGE_BASE: PolicyClause[] = [
  {
    clauseId: 'POLICY-001',
  ...
  {
    clauseId: 'POLICY-012',
    title: 'Driver Licensing & Operating Legality',
    policyText: 'The driver operating the insured motor vehicle at the time of loss must hold an effective, valid driving license issued by a competent licensing authority, appropriate for the specific class and category of the motor vehicle.',
    applicableClaimType: 'Both',
    conditions: [
      'Expired driving license without statutory renewal grace period voids coverage under own damage.',
      'Operating a four-wheeler with only a motorcycle endorsement voids own damage coverage.',
      'Learner permit holders must be accompanied by a licensed driver instructor.'
    ],
    evidenceRequired: 'Color copy of government driving license, digital verification record from national transport registry, and police accident verification statement.',
    category: 'Exclusion',
    description: 'Mandates active, valid driver license for vehicle class at exact date and hour of loss.',
    standardDeductionOrRule: 'Total claim repudiation if driver held no valid operating permit at time of loss.',
    appliesTo: 'Both',
    riskWeight: 'HIGH',
  }
];

// Helper to look up a policy clause by ID (supports legacy prefixes if needed)
export function getPolicyClauseById(clauseId: string): PolicyClause | undefined {
  const normalized = clauseId.toUpperCase().trim();
  const directMatch = MOTOR_POLICY_KNOWLEDGE_BASE.find(c => c.clauseId === normalized);
  if (directMatch) return directMatch;

  // Legacy mappings for backward compatibility
  const legacyMap: Record<string, string> = {
    'POL-OD-101': 'POLICY-001',
    'POL-DEP-102': 'POLICY-004',
    'POL-ADD-103': 'POLICY-009',
    'POL-EXC-201': 'POLICY-003',
    'POL-EXC-202': 'POLICY-012',
    'POL-EXC-203': 'POLICY-003',
    'POL-EXC-204': 'POLICY-003',
    'POL-CND-301': 'POLICY-008',
    'POL-CND-302': 'POLICY-005',
    'POL-DED-401': 'POLICY-004',
    'POL-EXC-402': 'POLICY-003',
    'POL-ADD-501': 'POLICY-011',
  };

  const mappedId = legacyMap[normalized];
  if (mappedId) {
    return MOTOR_POLICY_KNOWLEDGE_BASE.find(c => c.clauseId === mappedId);
  }
